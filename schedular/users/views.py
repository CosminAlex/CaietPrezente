...
from schedular.decorators import social_required
...
def get_social(request):
    return render(request, 'users/get_social.html', {
        'socials': User.objects.filter(is_social=True)
    })

def statisticaGrupe(request):
    return render(request, 'users/statisticaGrupe.html', {
        'socials': User.objects.filter(is_social=True)
    })
